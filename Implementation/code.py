# ************************************    Final Working Code with updated dataset Technova  ***************************************************
...
# Dictionary mapping sub-category labels to their corresponding texts
sub_category_texts = {
    0: "No Attack",
    1: "Analysis",
    2: "Backdoor",
...
def multiclass_prediction():
    ...
    xtest=pd.read_csv(testdata_file_path)
    #print(xtest)
    classifier = algorithms[best_algo]
    classifier.fit(X_train, y_train)
    y_pred = classifier.predict(xtest)
    y_pred = y_pred[0]  # Extracting the first prediction
    predicted_label_text = sub_category_texts[y_pred]
    e2.delete(0,'end')
    e2.insert(0, predicted_label_text)
